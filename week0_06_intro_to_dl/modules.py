...
class BatchNormalization(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        self.gradInput = 1/(input.shape[0] * np.sqrt(self.batch_variance + self.EPS)) * self.gradInput
        return self.gradInput
...
class Dropout(Module):
    ...
    def updateOutput(self, input):
        ...
        if self.training:
            self.mask = np.random.uniform(size=input.shape) < (1 - self.p)
...
#         pad_size = self.kernel_size // 2
        ...
        # self.output = ...
        self.output = np.zeros((input.shape[0], self.out_channels, *input.shape[2:]))
        for image_index, image in enumerate(input):
            for kernel_index, kernel in enumerate(self.W):
                self.output[image_index, kernel_index] = self.b[kernel_index]
                for channel_index, channel in enumerate(image):
                    ...
                    self.output[image_index, kernel_index] += convolution_matrix_kernel_channel
        
        return self.output
    
    def updateGradInput(self, input, gradOutput):
        self.gradInput = np.zeros(input.shape)
        for image_index, image in enumerate(gradOutput):
            for kernel_index, kernel in enumerate(self.W):
                for kernel_channel_index, kernel_channel in enumerate(kernel):
                    strided_downstream_gradient = np.zeros_like(image[kernel_index])
                    P = [0, 0]
                    for i in range(len(P)):
                        P[i] = input[image_index, kernel_channel_index].shape[i] - strided_downstream_gradient.shape[i] 
                        P[i] = (P[i] - ceil(P[i] / 2), ceil(P[i] / 2))
                    strided_padded_downstream_gradient_layer = np.pad(strided_downstream_gradient, P, mode='constant')
                    self.gradInput[image_index, kernel_channel_index] += convolve(strided_padded_downstream_gradient_layer, kernel_channel, mode='same', method='direct')
        return self.gradInput
    ...
    def accGradParameters(self, input, gradOutput):
        ...
        # self.gradW = ...
        # self.gradb = ...
        convolution_matrix_full = np.zeros(input.shape)
        gradient_weights = np.zeros_like(self.W)
        for image_index, image in enumerate(gradOutput):
            for kernel_index, kernel in enumerate(self.W):
                for kernel_channel_index, kernel_channel in enumerate(kernel):
                    strided_downstream_gradient = np.zeros_like(image[kernel_index])
                    ...
                    gradient_weights[kernel_index, kernel_channel_index] += correlate(input[image_index, kernel_channel_index], strided_padded_downstream_gradient_weights, mode='valid', method='direct')
        self.gradW += gradient_weights
        self.gradb += np.sum(gradOutput, axis=tuple(i for i in range(len(gradOutput.shape)) if i != 1))
        pass
